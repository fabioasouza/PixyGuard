...
finalize_installed_pixyguard() {
  ...
  /usr/bin/sudo /usr/libexec/PlistBuddy -c "Set :CFBundleIdentifier com.fabioasouza.pixyguard" "$APP/Contents/Info.plist"
...
complete_uninstall() {
  ...
  /bin/rm -f "$HOME/Library/Preferences/com.fabioasouza.pixyguard.plist" 2>/dev/null || true
  /usr/bin/defaults delete com.fabioasouza.pixyguard >/dev/null 2>&1 || true
  /usr/bin/defaults delete com.fabioasouza.pixyguard >/dev/null 2>&1 || true
...
echo "📦 Staging bundled PixyGuard source..."
mkdir -p \
  "$SRC/Sources/PixyBar" \
  "$SRC/Sources/pixyctl" \
  "$SRC/Resources" \
...
required_resources=(
  "$BUNDLED_MAIN"
  "$SCRIPT_RES/pixyctl.c"
  "$SCRIPT_RES/build.sh"
  "$SCRIPT_RES/Info.plist"
  "$SCRIPT_RES/AppIcon.icns"
)

for required in "${required_resources[@]}"; do
  [[ -f "$required" ]] || {
    echo "❌ Missing bundled installer resource: $required"
    exit 1
  }
done

/bin/cp "$BUNDLED_MAIN" "$SRC/Sources/PixyBar/main.swift"
/bin/cp "$SCRIPT_RES/pixyctl.c" "$SRC/Sources/pixyctl/pixyctl.c"
/bin/cp "$SCRIPT_RES/build.sh" "$SRC/scripts/build.sh"
/bin/cp "$SCRIPT_RES/Info.plist" "$SRC/Resources/Info.plist"
/bin/cp "$SCRIPT_RES/AppIcon.icns" "$SRC/Resources/AppIcon.icns"
/bin/chmod +x "$SRC/scripts/build.sh"

UPSTREAM_LICENSE="$SCRIPT_RES/LICENSE.txt"

echo "✅ Bundled PixyGuard source staged."
echo "🔨 Building PixyGuard locally from bundled source..."

(cd "$SRC" && ./scripts/build.sh)

BUILT="$SRC/PixyBar.app"
[[ -d "$BUILT" ]] || {
  echo "❌ PixyBar.app not found after local build"
  exit 1
}
...
echo "🔎 Verifying PIXY HID control..."
PIXYCTL="$APP/Contents/MacOS/pixyctl"

set +e
TEST_OUTPUT="$("$PIXYCTL" status 2>&1)"
RC=$?
set -e

print -r -- "$TEST_OUTPUT"

if [[ $RC -eq 0 ]] && ! print -r -- "$TEST_OUTPUT" | /usr/bin/grep -qiE 'could not open|not permitted|not available|error'; then
  echo "✅ PIXY HID control responded."
else
  echo "⚠️ PixyGuard installed, but PIXY HID could not be verified right now."
  echo "   Connect the PIXY over USB and reopen PixyGuard to retry."
fi
